...
function summary2(person) {
  ...
  return `${person.firstName} ${
    person.lastName
  } is a ${person.calcAge()}-year old ${person.job}, and ${
    person.hasDriversLicense
      ? `${pronoun} has a driver's license`
      : `${pronoun} does not have a driver's license`
  }`;
}
...
const yearsUntilRetirement = function () {
  ...
  if (birthYear < currentYear && firstName && retirementAge > age) {
    ...
    document.getElementById(
      "resultRetire"
    ).innerText = `The current year is ${currentYear}.
        ${firstName} plans to retire in ${retirement} years, in ${
      currentYear + retirement
    }.`;
  } else {
    document.getElementById(
      "resultRetire"
    ).innerText = `Please enter correct data.`;
  }
...
console.log(
  `The highest value is ${highLow[0]} and the lowest value is ${highLow[1]}.`
);

const highLow2 = calcHighAndLow(values2);

console.log(
  `The highest value is ${highLow2[0]} and the lowest value is ${highLow2[1]}.`
);
