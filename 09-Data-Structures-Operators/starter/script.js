...
const getCode = flight => flight.slice(0, 3).toUpperCase();
...
document.querySelector('button').addEventListener('click', function () {
  ...
  const results = [];
  for (const r of rows) {
    const newRow = r.toLowerCase().split('_');
    console.log(newRow);
  ...
  for (let i = 0; i < results.length; i++) {
    if (
      ...
      results[i].length > longestString.length
    ) {
      longestString = results[i];
    }
  }
  for (let i = 0; i < results.length; i++) {
    console.log(
...
console.log('Ann Nakajima'.split(' '));

const [firstName, lastName] = 'Ann Nakajima'.split(' ');
...
const passenger = 'ann yukiko nakajima';
...
capitalizeName('ann yukiko nakajima');
capitalizeName('daniel jonathan san pedro');

// Padding

const message = 'Go to gate 23!';
console.log(message.padStart(20, '+').padEnd(30, '+'));
console.log('Dan'.padStart(25, '+').padEnd(30, '+'));
...
console.log(maskedCreditCard(67894567));
console.log(maskedCreditCard(65276076071510));
console.log(maskedCreditCard('65276076071510'));
...
console.log('DanSanPedro'.length);
...
console.log('Unique:', uniqueItalianAndMexicanFoods);
...
for (const [key, scorer] of Object.entries(game.scored)) {
...
for (const [team, teamOdd] of Object.entries(game.odds)) {
